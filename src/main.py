...
def main():
    ...
    if (args.train and (not args.PROGRESSIVE)) :
        ...
        for backbone in ["resnet18", "resnet50"]:
            ...
            #path folder for logs and plots
            exp_name = f"{args.dataset}_{backbone}_{EPOCHS + int(args.EPOCHS)}ep"
            if args.attention:
                exp_name += "_attention"

            ###### if num of epochs different from global EPOCHS => plot a PROGRESSIVE training
            logger = TrainLogger(os.path.join("logs", exp_name), args.dataset, backbone, EPOCHS + int(args.EPOCHS), args.attention, args.no_eval)
            ...
            for epoch in range(int(args.EPOCHS)):
                loss = train_one_epoch(model, train_loader, optimizer, ce_loss, tri_loss, EPOCHS + epoch)
                print(f"Epoch [{EPOCHS + epoch + 1}/{EPOCHS + int(args.EPOCHS)}], Loss: {loss:.4f}")
                
                #for evaluation (query/gallery), logging and plotting 
                if args.no_eval:
                    logger.log(epoch= EPOCHS + epoch + 1, train_loss=loss, val_loss=0.0, rank1=0, map_score=0)
                    ...
                    logger.log(epoch=EPOCHS + epoch + 1, train_loss=loss, val_loss=0.0, rank1=rank1, map_score=mAP)
